...
void write_in_direction(coordinates_3d c3d, coordinates_3d direction, float distance, RGB **matrix, int dotted) {
    coordinates_2d c2d;
    img_coord img;
    int resolution = 999;
...
void write_cube(RGB **matrix, coordinates_3d reference_vertice, float size) { 
...
int main() {
    ...
    retrieve_small_matrix_from_txt("small_matrix.txt", small_matrix, small_x, small_y, small_depth);

    for (int n = 0; n < 2000; n++) {
        int random_x = 25+rand() % 10;
        int random_y = 25+rand() % 10;
        int random_z = 25+rand() % 10;
        small_matrix[random_x][random_y][random_z].r = 255;
    }
...
for (int sum = -small_depth; sum < small_x + small_y; sum++) {
        ...
        for (int j = 0; j < small_y; j++) {
            ...
            if (k >= 0 && k < small_depth) {
                if (small_matrix[i][j][k].r != 0) {
                    write_cube(matrix, (coordinates_3d){i-1, j-1, k-1}, 12);
    ...
    save_small_matrix_to_txt("small_matrix.txt", small_matrix, small_x, small_y, small_depth);
